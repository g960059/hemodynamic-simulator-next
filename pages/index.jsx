...
import { doc,collection,collectionGroup, getDocs, limit, orderBy, query, where, writeBatch } from 'firebase/firestore';
...
const TopPage = ({canvas}) => {
  ...
  return <Layout>
        <div className='w-full mx-auto px-4 md:px-10 text-base font-medium text-center text-slate-500 border-solid border-0 border-b border-slate-200'>
          <div className='flex flex-wrap -mb-px max-w-7xl '>
        ...
        }
      {
        tabValue=="mypage" && myCases.status=="success" && (
          ...
          <div className='max-w-4xl w-full mx-auto py-4 md:py-6 px-4 min-h-[440px]'>
            <div className="grid md:grid-cols-2 gap-4 md:gap-8">
              {
                myCases.data.map(c=><CanvasItem canvasItem={c} removeCanvas={removeCanvas} isOwner={true}/>)
              }
            </div>         
          </div>  :
          <div>
...
export const getStaticProps = async () => {
  ...
  const canvasSnap = await getDocs(query(collectionGroup(db,'canvas'),orderBy("updatedAt"),where("visibility", "==", "public"),limit(20)))
    ...
    <div key={canvasItem?.id} onClick={(e)=>{e.preventDefault();e.stopPropagation();router.push({pathname:`/canvas/${canvasItem?.id}`})}}  className="w-full flex flex-col py-3 px-4 bg-white cursor-pointer border border-solid border-slate-200 rounded-md overflow-hidden hover:shadow transition">
      ...
      <div className='ml-10 mt-2 flex items-center justify-center'>
        <span className='text-sm flex flex-row items-center justify-start text-slate-500'>
          ...
          <span className='text-sm ml-0.5'>{canvasItem?.heartCount || 0}</span>
        </span>
        <div className='flex-grow'/>
        {
          isOwner && (
