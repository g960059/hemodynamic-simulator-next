...
import { useCreateBlockNote, ReactSlashMenuItem, getDefaultReactSlashMenuItems,createReactBlockSpec,InlineContent, SuggestionMenuController } from "@blocknote/react";
import { BlockNoteView, lightDefaultTheme } from "@blocknote/mantine";
import { BlockNoteSchema, defaultBlockSchema, defaultBlockSpecs, defaultInlineContentSpecs, defaultProps, defaultStyleSpecs, filterSuggestionItems} from "@blocknote/core";
...
const NotePanel = React.memo(({ view = null,updateView,removeView, isOwner,caseData, setCaseData}) => {
  ...
  const [localAllImagesInStorage, setLocalAllImagesInStorage] = useState(caseData?.allImagesInStorage || []);
...
const MathBlock = createReactBlockSpec({
  ...
  render: ({ block, editor }) => {
    ...
    const [equation, setEquation] = useState(block.props.equation);
    ...
    useEffect(() => {
      // 新しく追加されたMathBlockを自動的に編集モードにする
      if (block.props.equation === "" && !isEditing) {
        setIsEditing(true);
      }
    }, []);
    ...
    return (
      <div
        onClick={handleClick}
        className='w-full py-2 px-4 my-2 rounded-lg transition-all duration-200 ease-in-out'
        style={{
          backgroundColor: isEditing ? 'rgba(59, 130, 246, 0.1)' : 'transparent',
          border: isEditing ? '1px solid rgba(59, 130, 246, 0.5)' : '1px solid transparent',
        }}
      >
        {isEditing ? (
          <textarea
            ...
            className='w-full p-2 bg-transparent border-none outline-none resize-none font-mono text-sm'
            placeholder="\int_0^\infty x^2 dx"
            rows={3}
            autoFocus
          />
          ...
          <div className='cursor-text'>
            {equation ? (
              ...
              <div className='text-sm text-slate-400 flex items-center justify-center h-16'>
