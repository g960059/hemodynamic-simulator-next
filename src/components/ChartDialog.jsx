    ...
    <Dialog fullScreen={!isUpMd} sx={{ ".MuiDialog-paper": {m:0}}} open ={open} onClose ={onClose}>
      <div className='sticky top-0 md:min-w-[460px] bg-white border-solid border-0 border-b border-slate-200 w-full p-3 pl-4 flex flex-row items-center justify-center'>
        <div className='text-base font-bold text-center inline-flex items-center'>
          <svg className='w-6 h-5 mr-1.5 stroke-blue-500' xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor">
      ...
      <div className='w-full px-6 py-5'>
        ...
        { (view.type == "PressureCurve" ||  view.type == "FlowCurve") && <>
            <div className='flex flex-row items-center w-full mt-1'>
              ...
              >
                {[2,4,6,12,30].map(tw=><MenuItem key={tw} value={tw}>{tw}秒</MenuItem>)}
              </Select>          
            </div>
          </>
        } 
...
const SortableItem = ({ id, item, index, patients,view, setView, edittingIndex, setEdittingIndex, setOpenNewItem, setViewItemAnchorEl, setActiveItemId, viewItemAnchorEl, activeItemId }) => {
