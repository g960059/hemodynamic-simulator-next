...
import {Box,Typography,Grid,Tab,Tabs, Divider,AppBar,Tooltip, Toolbar,Button,IconButton,Stack,Switch,Dialog,DialogContent,DialogActions,DialogTitle,TextareaAutosize,useMediaQuery,Slide} from '@mui/material'
...
const Chapter = () => {
  ...
  const [chapterData, setChapterData] = useImmer({});
  ...
  const isChanged = loadedChapter.status == "success" && (loadedChapter.data &&
    (!isEqual(objectWithoutKeys(chapterData,["updatedAt","createdAt"]),objectWithoutKeys(loadedChapter.data,["updatedAt","createdAt"]))) || !loadedChapter.data);

  useEffect(() => {
    setLoading(true)
    if(loadedChapter.status == "success" && loadedChapter.data?.body){
      setChapterData(loadedChapter.data)
    }else{
    ...
    setLoading(false)
  }, [loadedChapter.status]);


  const updateChapter = async () =>{
    const timestamp = serverTimestamp();
    if(!loadedChapter.data?.createdAt){
      await setDoc(doc(db,'users',user?.uid,"books",router.query.bookId,"chapters",router.query.chapterId),{...chapterData,updatedAt:timestamp,createdAt:timestamp})
    }else{
      await updateDoc(doc(db,'users',user?.uid,"books",router.query.bookId,"chapters",router.query.chapterId),{...chapterData,updatedAt:timestamp})
    }
  }
  ...
  return <>
      <AppBar position="static" elevation={0} className={classes.appBar} classes={{root:classes.appBarRoot}}>
        <Toolbar>
          <Box onClick={()=>{router.push(`/books/${router.query.bookId}`)}} sx={{cursor:"pointer",fontFamily: "GT Haptik Regular" ,fontWeight: 'bold',display:"flex",ml:{xs:0,md:2}}}>
            <IconButton><ArrowBack/></IconButton>
          </Box>
          <div style={{flexGrow:1}}/>
          <Switch 
            checked={chapterData.visibility!="private"} 
            onChange={e=>{if(e.target.checked){
                setChapterData(draft =>{draft.visibility="public"})
              }else{
                setChapterData(draft =>{draft.visibility="private"})
            }}}/>
          <Typography variant='button' sx={{color:chapterData.visibility!="private" ? "black": "gray", mr:1}}>{t["Publish"]}</Typography>
          <Button 
            variant='contained' 
            disableElevation 
          ...
          >
            {isChanged ? (chapterData.visibility=="private" ? "下書き保存" : (chapterData.visibility!="private" && loadedChapter.data?.visibility!="private" ? "保存する" : t["Publish"] )) : t["Saved"]}
          </Button>
      ...
      <div className='w-full max-w-4xl mx-auto'>
        <div className="pt-7 w-full">
          <TextareaAutosize
            ...
            onChange={e=>{setChapterData(draft=>{draft.name=e.target.value ?? "Title"})}} 
            placeholder="Title"
            className="appearance-none w-full font-bold mx-2 text-xl md:text-3xl py-1 px-1 border-0 bg-transparent placeholder-slate-400 focus:outline-none hover:bg-blue-50 resize-none" 
          />
        </div>
        {loading && <Box>
        ...
        {loadedChapter.status=="success" &&chapterData.body&& !loading && (isUpMd != null && isUpMd != undefined) && <>
          <Stack direction="row" pb={{xs:10,md:8}}>
            <Box className={classes.plateBox}>
              <Plate 
                id={"chapter_"+router.query.chapterId} 
                editableProps={editableProps} 
                initialValue={chapterData.body} 
                onChange={newValue=>{setChapterData(draft=>{draft.body=newValue})}}
                plugins={createPlugins([
                  ...
                  createImagePlugin({
                    uploadImage: (file) => {
                      ...
                      const storageRef = ref(storage, `images/${user.uid}/books/${router.query.bookId}/chapters/${router.query.chapterId}/${imageId}`);
...
export const BasicElementToolBarButtons = ()=>{
  const router = useRouter()
  const editor = usePlateEditorRef("chapter_"+router.query.chapterId);
  return (
    ...
    <label htmlFor={`upload-button`}>
      <input
        ...
        onChange={async (e)=>{
          ...
          const imageId = nanoid()
          const storageRef = ref(storage, `images/${auth.currentUser.uid}/books/${router.query.bookId}/chapters/${router.query.chapterId}/${imageId}`);
