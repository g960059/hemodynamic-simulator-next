...
import {Typography,Button,Stack, Avatar, useMediaQuery,NoSsr, Dialog} from '@mui/material'
...
function UserSummary(){
  ...
  const uid$ = of(queryParams.userId).pipe(
    filter(Boolean),
    mergeMap(userId=> docData(doc(db,'userIds',userId))),
    map(user => user.uid)
  );
  const {data:uid} = useObservable(`uid_${queryParams.userId}`, uid$);
  const {data:user} = useObservable(`user_${queryParams.userId}`,uid$.pipe(
    filter(Boolean),
  ...
  const {data: followerIds} = useObservable(`followerIds_${queryParams.userId}`, followerIds$);
  const followingIds$ = uid$.pipe(
    filter(Boolean),
    mergeMap(uid => collectionData(collection(db, 'users', uid, 'following'))),
  );
  const {data: followingIds} = useObservable(`followingIds_${queryParams.userId}`, followingIds$);
  ...
  const { data: followers } = useObservable(`followersWithDetails_${queryParams.userId}`, followersWithDetails$);
  ...
  const { data: followings } = useObservable(`followingWithDetails_${queryParams.userId}`, followingWithDetails$);
